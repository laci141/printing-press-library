...
In local mode: searches locally synced data only.`,
  ...
  clinical-trials-pp-cli search "error timeout"

  # Force local search only
  clinical-trials-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  clinical-trials-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  clinical-trials-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("clinical-trials-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'clinical-trials-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "studies":
				results, err = db.SearchStudies(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchStudies(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search studies failed: %w", searchErr)
					}
