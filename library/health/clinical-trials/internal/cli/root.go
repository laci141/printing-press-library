...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "clinical-trials-pp-cli",
		Short: `Clinical Trials CLI — A multi-source clinical-trials intelligence system — aggregates, normalizes, and analyzes trials across ClinicalTrials.…`,
		Long: `Clinical Trials CLI — A multi-source clinical-trials intelligence system — aggregates, normalizes, and analyzes trials across ClinicalTrials.…
...
Health check: run 'clinical-trials-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("clinical-trials-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'clinical-trials-pp-cli profile list')")
	...
	rootCmd.AddCommand(newStudiesCmd(flags))
	...
	rootCmd.AddCommand(newNovelHealthCmd(flags))
	...
	rootCmd.AddCommand(newNovelEnrollmentCheckCmd(flags))
	rootCmd.AddCommand(newNovelExportFHIRCmd(flags))
	...
	rootCmd.AddCommand(newHotspotsCmd(flags))
	rootCmd.AddCommand(newPhase3Cmd(flags))
	rootCmd.AddCommand(newRecruitingCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newClinicaltrialsGovVersionPromotedCmd(flags))
