...
Resource scoping:
  ...
  nejm-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nejm-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nejm-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nejm-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nejm-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nejm-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "article":
		...
		return db.UpsertArticle(typedData)
	case "specialty":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertSpecialty(typedData)
	default:
