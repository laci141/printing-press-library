...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "nejm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NEJM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NEJM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nejm-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/nejm-pp-cli/feedback.jsonl.
When ` + "`NEJM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NEJM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nejm-pp-cli feedback list
  nejm-pp-cli feedback list --limit 5
  nejm-pp-cli feedback list --json`,
