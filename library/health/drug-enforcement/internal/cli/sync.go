...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/health/drug-enforcement/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/health/drug-enforcement/internal/store"
...
Resource scoping:
  ...
  drug-enforcement-pp-cli sync

  # Sync specific resources only
  drug-enforcement-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  drug-enforcement-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  drug-enforcement-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  drug-enforcement-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  drug-enforcement-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("drug-enforcement-pp-cli")
			}
