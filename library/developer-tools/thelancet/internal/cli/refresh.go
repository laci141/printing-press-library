...
func newLancetRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fromBound, toBound, err := refreshYearBounds(cmd, years, fromYear, toYear)
			if err != nil {
				return err
			}
...
func refreshYearBounds(cmd *cobra.Command, years, fromYear, toYear int) (int, int, error) {
	if years > 0 && (fromYear > 0 || toYear > 0) {
		_ = cmd.Usage()
