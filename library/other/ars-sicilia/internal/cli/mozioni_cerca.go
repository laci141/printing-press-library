...
func newMozioniCercaCmd(flags *rootFlags) *cobra.Command {
	var (
		flagLegisl     int
	...
	cmd := &cobra.Command{
		...
		Short:   "Cerca mozioni parlamentari.",
		Example: "  ars-sicilia-pp-cli mozioni cerca --legisl 18 --json",
		Annotations: map[string]string{
			"pp:endpoint":   "mozioni.cerca",
			"mcp:read-only": "true",
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runCerca(cmd, flags, "mozioni", cercaParams{
