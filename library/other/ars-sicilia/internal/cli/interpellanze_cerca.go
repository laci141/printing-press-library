...
func newInterpellanzeCercaCmd(flags *rootFlags) *cobra.Command {
	var (
		flagLegisl     int
	...
	cmd := &cobra.Command{
		...
		Short:   "Cerca interpellanze parlamentari.",
		Example: "  ars-sicilia-pp-cli interpellanze cerca --legisl 18 --json",
		Annotations: map[string]string{
			"pp:endpoint":   "interpellanze.cerca",
			"mcp:read-only": "true",
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runCerca(cmd, flags, "interpellanze", cercaParams{
