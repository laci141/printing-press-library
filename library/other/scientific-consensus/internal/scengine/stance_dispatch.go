...
// automatically. It returns the stance, confidence (0..1), and the method
...
func ClassifyStanceAuto(title, abstract, claim string) (Stance, float64, string) {
	if LLMConfigured() {
		if st, conf, err := ClassifyStanceWithLLM(title, abstract, claim); err == nil {
