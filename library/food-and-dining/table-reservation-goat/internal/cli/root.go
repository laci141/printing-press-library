...
type rootFlags struct {
	...
	// invocation. Mirrors the TABLE_RESERVATION_GOAT_NO_LEARN env var.
	noLearn bool
...
Run 'table-reservation-goat-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("table-reservation-goat-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'table-reservation-goat-pp-cli profile list')")
	...
	rootCmd.AddCommand(newRestaurantsCmd(flags))
	...
	// Hand-written cross-network novel commands (auth/goat/watch/earliest/
	// drift/book/cancel/location). See .printing-press-patches/ (novel-commands,
	// resy-source-port, location-native-redesign). They live alongside the
	// generated OpenAPI reservations/experiences commands, not in place of them.
