# Changelog

This file is maintained by printing-press-library release automation. Do not hand-edit release sections in normal PRs.

## 2026.8.1 - 2026-08-06

- fix(table-reservation-goat): fetch OpenTable availability bodies after loadingFinished with bounded retry (#1555).

## 2026.7.6 - 2026-07-20

- fix(table-reservation-goat): Tock attach-mode booking on the modern SPA layout (#1514).

## 2026.7.5 - 2026-07-18

- feat(table-reservation-goat): OpenTable attach-mode booking (#1488).

## 2026.7.4 - 2026-07-13

- feat(table-reservation-goat): add table-reservation-goat (#1510).

## 2026.7.3 - 2026-07-08

- feat: registry hygiene follow-ups (pp-first descriptions, expanded MCP validation, --validate for skills, npm publish auto-trigger) (#675).

## 2026.7.2 - 2026-07-08

- fix(table-reservation-goat): self-heal stale OpenTable availability persisted-query hash (#1448).

## 2026.7.1 - 2026-07-08

- fix(catalog): require Go 1.26.5 across published modules (#1467).

## 2026.6.2 - 2026-06-21

- fix(catalog): require Go 1.26.4 across published modules (#1308).

## 2026.6.1 - 2026-06-08

- Baseline release metadata added for this published CLI.
